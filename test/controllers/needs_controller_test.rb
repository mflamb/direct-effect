...
class NeedsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create need" do
    assert_difference('Need.count') do
      post needs_url, params: { need: { category: @need.category, comment: @need.comment, enabled: @need.enabled, item: @need.item, organization_id: @need.organization_id } }
    end
  ...
  test "should update need" do
    patch need_url(@need), params: { need: { category: @need.category, comment: @need.comment, enabled: @need.enabled, item: @need.item, organization_id: @need.organization_id } }
